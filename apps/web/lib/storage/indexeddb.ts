...
export async function storePrivateKey(privateKey: Uint8Array, password: string): Promise<void> {
  ...
  const encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, encKey, toBuffer(privateKey));
...
export async function loadPrivateKey(password: string): Promise<Uint8Array | null> {
  ...
  try {
    const decrypted = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: toBuffer(iv) }, encKey, toBuffer(ciphertext));
