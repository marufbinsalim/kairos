  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl w-full max-w-xl shadow-2xl flex flex-col max-h-[80vh]">
            ...
            <>
                ...
                {(['paste', 'file'] as const).map((t) => (
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-md text-sm font-medium transition-colors ${
                      tab === t ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'
                    }`}
                  >
              ...
              {tab === 'paste' ? (
                ...
                >
                  ...
                  <p className="text-gray-600 dark:text-gray-400 text-sm font-medium">Click to upload <span className="text-gray-500 font-normal">or drag and drop</span></p>
                  <p className="text-gray-500 dark:text-gray-600 text-xs mt-1">.env files only</p>
              ...
              <div className="space-y-1.5">
                {parsed.map((row, i) => (
                  <div key={i} className="flex items-center gap-3 bg-gray-100 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700/60 rounded-lg px-3 py-2">
                    ...
                    <button
                      onClick={() => removeRow(i)}
                      className="text-gray-400 dark:text-gray-600 hover:text-red-500 dark:hover:text-red-400 transition-colors shrink-0"
                    >
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-800 shrink-0 flex gap-3">
          {parsed ? (
            <>
              <button
      ...
      <div className="flex items-center gap-3 bg-white dark:bg-gray-900 border border-indigo-500/40 rounded-xl px-5 py-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="w-7 h-7 flex items-center justify-center rounded-lg text-green-500 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-950/30 disabled:opacity-50 transition-colors"
            title="Save"
          >
            {saving ? (
    ...
    <div className="flex items-center gap-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 rounded-xl px-5 py-3.5 transition-colors group">
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <button onClick={handleDelete} disabled={deleting} title="Delete"
          className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 disabled:opacity-30 transition-colors">
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    <div className="mt-10">
      ...
      {token ? (
        <div className="flex items-center gap-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-5 py-3.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="text-xs text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 px-3 py-1.5 rounded-lg border border-gray-300 dark:border-gray-700 transition-colors"
            >
              Revoke
            </button>
          </div>
        ...
        <div className="flex items-center justify-between">
          <p className="text-gray-500 text-sm">No deploy token for this environment.</p>
          <button
            onClick={handleGenerate}
            disabled={!dek || generating}
    ...
    <AppShell>
      <div className="px-4 sm:px-8 py-8 max-w-4xl mx-auto">
        ...
        <nav className="flex items-center gap-2 text-sm mb-6">
          ...
          <Link href={`/dashboard/projects/${params.projectId}`} className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">
            {env?.projectName ?? 'Project'}
          </Link>
          <svg className="w-3.5 h-3.5 text-gray-400 dark:text-gray-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
          </svg>
          <span className="text-gray-700 dark:text-gray-300 font-medium">{env?.name ?? 'Environment'}</span>
        </nav>

        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Secrets</h1>
        ...
        {syncing && (
          <div className="mb-5 flex items-center gap-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-3">
            ...
            <span className="text-gray-500 dark:text-gray-400 text-sm">Loading encryption key…</span>
          </div>
        )}
        ...
        {isLoading ? (
          ...
          <>
            ...
            <p className="text-gray-400 dark:text-gray-600 text-xs mt-4 text-right">{secrets.length} secret{secrets.length !== 1 ? 's' : ''}</p>
          </>
        )}

        <DeployTokensSection envId={params.envId} dek={dek} />
      </div>
