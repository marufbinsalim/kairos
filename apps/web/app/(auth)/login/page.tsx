...
function LoginInner() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black flex">
      {/* Brand panel */}
      <div className="hidden lg:flex w-[44%] flex-col justify-between border-r border-gray-200 dark:border-gray-900 bg-gray-50 dark:bg-gray-950 p-10 xl:p-14">
        <BrandLink size={30} />
        <div className="max-w-md">
          <h2 className="text-3xl xl:text-4xl font-bold tracking-tighter text-gray-900 dark:text-white leading-[1.1] mb-8">
            Secrets your team can share.<br />
            <span className="text-gray-500">Keys only you hold.</span>
          </h2>
          <ul className="space-y-4 mb-10">
            {[
              'Encrypted on your device before anything syncs',
              'New devices wait for approval from a trusted one',
              'One CLI command pulls secrets into any environment',
            ].map((line) => (
              <li key={line} className="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-400">
                <svg className="w-4 h-4 mt-0.5 text-green-600 dark:text-green-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-black px-4 py-3 font-mono text-[13px]">
            <span className="text-gray-400 dark:text-gray-600">$</span>{' '}
            <span className="text-gray-900 dark:text-gray-100">kairos run -- npm start</span>
            <span className="block text-gray-500 mt-1">  ▸ injected 12 secrets · acme-app › production</span>
          </div>
        </div>
        <p className="text-xs text-gray-400 dark:text-gray-600 font-mono">AES-256-GCM · x25519 · zero knowledge</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4 relative">
        <button
          onClick={toggle}
          className="absolute top-5 right-5 w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors"
        >
          {theme === 'dark' ? (
